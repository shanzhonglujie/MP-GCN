...
def proceed_dict1():
    ...
    x1 = Embedding(input_dim=num_words, output_dim=128, input_length=np.shape(x_train)[1], name='word_embedding')(main_input)
    cnn1 = Conv1D(64, 3, padding='same', strides=1, activation='relu')(x1)
    cnn1 = MaxPooling1D(pool_size=16)(cnn1)
    cnn2 = Conv1D(32, 4, padding='same', strides=1, activation='relu')(x1)
    cnn2 = MaxPooling1D(pool_size=16)(cnn2)
    cnn3 = Conv1D(32, 5, padding='same', strides=1, activation='relu')(x1)
    cnn3 = MaxPooling1D(pool_size=16)(cnn3)
    x1 = concatenate([cnn1, cnn2, cnn3], axis=-1)
    # 合并三个模型的输出向量
